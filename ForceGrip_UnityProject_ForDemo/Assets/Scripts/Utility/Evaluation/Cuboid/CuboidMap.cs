...
public class CuboidMap {
	...
	public void SenseSingleCollider(Matrix4x4 pivot, LayerMask mask, Vector3 size, Collider specificCol, float smoothing=0f, bool extractBoundary = false) {
        ...
        if(Physics.OverlapBox(pivotPosition, Size / 2, pivotRotation, mask).Length == 0)
        {
        ...
        for(int i=0; i<Points.Length; i++) {
            if(Size == Vector3.zero) {
                ...
                Collider c;
                Vector3 closest = Utility.GetClosestPointOverlapBoxSpecificCollider(sensor, step/2f, sensorRotation, mask, out c, specificCol);
                Occupancies[i] = smoothing*Occupancies[i] + (1f-smoothing)*(c == null ? 0f : 1f - Vector3.Distance(sensor, closest) / (range * Mathf.Sqrt(3) / 2 )); //2분의 루트 3 * range
            }
        }
    ...
    public void Sense(Matrix4x4 pivot, LayerMask mask, Vector3 size, float smoothing=0f, bool extractBoundary = false) {
        ...
        for(int i=0; i<Points.Length; i++) {
            if(Size == Vector3.zero) {
                ...
                References[i] = pivotPosition + pivotRotation * Vector3.Scale(Points[i], Size);
                Vector3 sensor = sensorPosition + sensorRotation * Vector3.Scale(Points[i], Size);
                Vector3 closest = Utility.GetClosestPointOverlapSphere(sensor, range * Mathf.Sqrt(3f) / 2f, mask, out bool found);
                Occupancies[i] = smoothing*Occupancies[i] + (1f-smoothing)*(!found ? 0f : 1f - Vector3.Distance(sensor, closest) / (range * Mathf.Sqrt(3f) / 2f ));
                if (Occupancies[i] < 0f)
                    Occupancies[i] = 0;
            }
        }
    ...
    public void Draw(Color colorOccupied) {
        ...
        UltiDraw.Begin();
        Vector3 step = GetStep();
        // UltiDraw.DrawCube(position, rotation, 10f, colorOccupied);
        if (Size != Vector3.zero) {
            ...
            for(int i=0; i<Points.Length; i++) {
                ...
                else if(Occupancies[i] < 0f)
                {
                    UltiDraw.DrawCuboid(References[i], rotation, step, Color.Lerp(UltiDraw.None, Color.red, -Occupancies[i]));
                }
                else
                {
                    UltiDraw.DrawCuboid(References[i], rotation, step, new Color(1f, 1f, 1f, 0.025f));
                }
            }
		}
