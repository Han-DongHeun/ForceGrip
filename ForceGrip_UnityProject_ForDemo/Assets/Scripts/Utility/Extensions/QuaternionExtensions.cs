...
public static class QuaternionExtensions {
	...
	public static Quaternion GetRelativeRotationTo(this Quaternion rotation, Quaternion to) {
		return Quaternion.Inverse(rotation) * to;
	}
  ...
  if(fabs(b) <= _QUATERNION_EPS*fabs(q.w)) {
	...
	*/
    ...
    public static Vector3 CalculateEulerVelocity_Deg(this Quaternion from, Quaternion to, float dt=1f)
    {
	    // Calculate the relative rotation from q1 to q2
	    Quaternion deltaQ = to * Quaternion.Inverse(from);
	    
	    // Convert to angular velocity
	    return deltaQ.eulerAngles.LimitEulerRanges() / dt;
    }
}
